...
angular.module('avatar', ['angularFileUpload'])
    ...
    .directive("avatar", function (FileUploader, $window, $rootScope, $log, ENV) {
        'use strict';
        return {
            restrict: 'AE',
            scope: {
            ...
            controller: function ($scope, FileUploader, $modal) {
                ...
                $scope.getAvatar = function (avatar) {
                    return (avatar) ? ENV.apiEndPoint + '/file/Person/' + avatar : 'imgs/user.png';
                };
                $scope.modifyAvatar = function () {
                    modalOpened = true;
                    modal = $modal.open({
                        templateUrl: 'templates/directives/setAvatarIndividuModal.html',
                ...
                $scope.$watch('person', function (newValue, oldValue) {
                    if (angular.isDefined(newValue) && !angular.equals(oldValue, newValue)) {
                        $scope.uploader.url = ENV.apiEndPoint + '/file/Person/avatar/' + newValue._id;
                    }
                });
    ...
    .directive("simpleAvatar", function (ENV) {
        'use strict';
        return {
            restrict: 'AE',
            scope: {
            ...
            templateUrl: 'templates/directives/simpleAvatar.html'
        };
    });